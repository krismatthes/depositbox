...
export default function ContractViewPage() {
  ...
  const [signatures, setSignatures] = useState<any>({ tenant: null, landlord: null })
  ...
  useEffect(() => {
    ...
    if (user && params.id) {
      ...
      const tenantSignature = localStorage.getItem(`signature_tenant_${contractId}`)
      const landlordSignature = localStorage.getItem(`signature_landlord_${contractId}`)
      
      let tenantSig = null
      let landlordSig = null
      
      try {
        if (tenantSignature) tenantSig = JSON.parse(tenantSignature)
        if (landlordSignature) landlordSig = JSON.parse(landlordSignature)
      } catch (error) {
        console.error('Failed to parse signatures:', error)
      }
      
      setSignatures({ tenant: tenantSig, landlord: landlordSig })
      
      // Try to load contract from created_contracts
      ...
      // Contract must exist in created_contracts to be viewed
      if (!foundContract) {
  ...
  const fullyExecuted = signatures.tenant && signatures.landlord
        ...
        <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-slate-800">Lejekontrakt</h1>
                <p className="text-slate-600 mt-1">{contract.propertyDetails?.address || contract.propertyAddress}</p>
              </div>
            </div>
            ...
            <div className={`p-4 rounded-xl border ${
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className={`font-semibold ${fullyExecuted ? 'text-green-800' : 'text-amber-800'}`}>
                    {fullyExecuted ? 'Kontrakt fuldt underskrevet' : 'Afventer underskrifter'}
                  </div>
                  <div className={`text-sm ${fullyExecuted ? 'text-green-700' : 'text-amber-700'}`}>
                    {fullyExecuted 
                      ? 'Denne kontrakt er juridisk bindende og aktiv'
                      : 'Kontrakten bliver aktiv når begge parter har underskrevet'
                    }
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden mb-8">
            ...
            <div className="p-8 space-y-8 text-sm leading-relaxed">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="bg-slate-50 p-4 rounded-lg">
                      <p className="font-medium">{contract.propertyDetails?.address || contract.propertyAddress}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="bg-slate-50 p-4 rounded-lg space-y-2">
                      <p><strong>Størrelse:</strong> {contract.propertyDetails?.size || contract.propertySize || 'Ikke angivet'} m²</p>
                      <p><strong>Antal værelser:</strong> {contract.propertyDetails?.rooms || contract.rooms || 'Ikke angivet'}</p>
                      <p><strong>Etage:</strong> {contract.propertyDetails?.floor || contract.floor || 'Ikke angivet'}</p>
                      <p><strong>Elevator:</strong> {(contract.termsAndConditions?.elevator || contract.elevator) ? 'Ja' : 'Nej'}</p>
                      <p><strong>Møbleret:</strong> {(contract.termsAndConditions?.furnished || contract.furnished) ? 'Ja' : 'Nej'}</p>
                    </div>
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <p><strong>Lejeforhold påbegyndes:</strong> {formatDate(contract.leaseTerms?.startDate || contract.startDate)}</p>
                  </div>
                  <div>
                    <p><strong>Lejeforhold ophører:</strong> {formatDate(contract.leaseTerms?.endDate || contract.endDate)}</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="bg-slate-50 p-4 rounded-lg">
                        <p className="text-xl font-bold text-blue-600">{formatCurrency((contract.financials?.monthlyRent || contract.monthlyRent || 0) * 100)} DKK</p>
                    ...
                    <div>
                      ...
                      <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                        <p className="text-xl font-bold text-green-600">{formatCurrency((contract.financials?.deposit || contract.deposit || 0) * 100)} DKK</p>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <p>Da dette er et <strong>tidsbegrænset lejemål</strong>, udløber kontrakten automatisk den {formatDate(contract.leaseTerms?.endDate || contract.endDate)} uden yderligere opsigelse.</p>
              ...
              <div className="p-6 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <div className="text-blue-700 space-y-2">
                  <p>Depositum på {formatCurrency((contract.financials?.deposit || contract.deposit || 0) * 100)} DKK er sikret gennem Nest Escrow Service, som fungerer som neutral tredjepart.</p>
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
            <div className="bg-gradient-to-r from-slate-600 to-slate-700 px-6 py-4">
              <h2 className="text-xl font-bold text-white">Digitale Underskrifter</h2>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="p-4 bg-slate-50 rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-slate-800">Udlejer</h3>
                  {signatures.landlord ? (
                    <div className="flex items-center gap-2 text-green-600">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                      </svg>
                      <span className="text-sm font-semibold">Underskrevet</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2 text-amber-600">
                      ...
                      <span className="text-sm font-semibold">Afventer</span>
                    </div>
                  )}
                </div>
                <div className="text-sm text-slate-600">
                  <div><span className="font-medium">{contract.landlord.firstName} {contract.landlord.lastName}</span></div>
                  <div>{contract.landlord.email}</div>
                  {signatures.landlord && (
                    <div className="text-green-600 mt-2">
                      Underskrevet: {new Date(signatures.landlord.signedAt).toLocaleDateString('da-DK', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric',
                        hour: '2-digit',
              ...
              <div className="p-4 bg-slate-50 rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-slate-800">Lejer</h3>
                  {signatures.tenant ? (
                    <div className="flex items-center gap-2 text-green-600">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                ...
                <div className="text-sm text-slate-600">
                  <div><span className="font-medium">{contract.tenant.firstName} {contract.tenant.lastName}</span></div>
                  <div>{contract.tenant.email}</div>
                  {signatures.tenant && (
                    <div className="text-green-600 mt-2">
                      Underskrevet: {new Date(signatures.tenant.signedAt).toLocaleDateString('da-DK', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric',
                        hour: '2-digit',
              ...
              <div className="flex justify-center gap-4 pt-4 border-t border-slate-200">
                {!signatures.tenant && user?.role === 'TENANT' && (
                  <button
                ...
                {!signatures.landlord && user?.role === 'LANDLORD' && (
                  <button
                    onClick={() => router.push(`/contracts/${contract.id}/sign`)}
                    className="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors"
                  >
